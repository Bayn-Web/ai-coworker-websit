...
@media (max-width: 768px) {
  ...
  /* 移动端优化的glitch效果 - 保持动画但使用简化版本 */
  @keyframes glitch-anim-mobile {
    0%, 100% {
      clip: rect(0, 9999px, 20%, 0);
    }
    25% {
      clip: rect(20%, 9999px, 40%, 0);
    }
    50% {
      clip: rect(40%, 9999px, 60%, 0);
    }
    75% {
      clip: rect(60%, 9999px, 80%, 0);
    }
  }
  ...
  .glitch-text.mobile-optimized::before {
    animation: glitch-anim-mobile 5s infinite linear alternate-reverse;
  }
  
  .glitch-text.mobile-optimized::after {
    animation: glitch-anim2-mobile 5s infinite linear alternate-reverse;
  }
}
